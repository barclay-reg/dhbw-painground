...
public class PaingroundApplicationTests extends FluentTest {
	...
	private String WAIT_FOR_VAADIN_SCRIPT =
	...
	public void loadPageAndMakeScreenshot() throws IOException {
		...
		//await().atMost(60, TimeUnit.SECONDS).until($(".v-loading-indicator")).not().present();
		waitForVaadin();
		
		File screenshot = webDriver.getScreenshotAs(OutputType.FILE);
		FileUtils.copyFile(screenshot, new File("target/screenshot1.png"));
	}
